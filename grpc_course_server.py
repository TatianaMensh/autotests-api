...
class CourseService(course_service_pb2_grpc.CourseServiceServicer):
    ...
    def GetCourse(self, request, context):
        ...
        print(f'Получен запрос к методу GetCourse с номером: {request.course_id}')

        # Формируем и возвращаем ответное сообщение
        title = "Автотесты API"
        description = "Будем изучать написание API автотестов"
        return course_service_pb2.GetCourseResponse(message=f"{request.course_id} {title}. {description}.")
